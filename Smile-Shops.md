...
1. The first step in adding a new store is to collect all of the necessary data needed to populate all of the required fields on the Calendar sheet. Normally all of the information needed will be provided on the ticket requesting the store be added. A description of the fields can be found above.
...
3. Once ready, run on staging. Get Jessica, Jordan, and QA to review. If you want to be fancy, you'll need to block out the first week of slots. Generally, you are making the store go live a week before they want anyone booked. Slots will get generated for that week in-between so you will want to close them out (can be done from the shell). Another option is to change the store hours so lunch blocks out all the slots, run pyrunner. Generate slots for the next week. Change the hours back to what is requested, run pyrunner, generate slots for the go live date and following weeks. 
4. Slots will not be bookable from the ecommerce website until the zipcodes are active. To add zipcodes to the stores they need to be added to the ServiceAreas sheet. 
...
THINGS TO KNOW:
* We have a nightly task (DailySchedulingSlotsTask) that runs and generates slots for the next 18 days for all the slots. Sometimes this has unexpected consequences if adding new hours, stores, or chairs. This task is located in core/locations/tasks.py  
